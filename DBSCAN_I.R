library(dbscan)
library(readxl)
library(dplyr)
library(e1071)
library(Boruta)
library(caret)
library(raster)
library(dismo)
library(openxlsx)

setwd("E:/")
file = read_excel("dataset.xlsx")
# file =read.csv("FRCI_Line_6.csv")
head(file)
dataall <- file[,-c(1,2,4,6,13)] ## Drop column Band_1 and Band_9 in dataframe
data <- file[,-c(7,8)] ## Drop column Band_1 and Band_9 in dataframe
head(dataall)
data <- dataall
data <- file

number <- data %>%
  group_by(Class) %>%
  summarize(n())
sample <- data%>%
  group_by(Class)%>%
  sample_n(min(number$`n()`))
head(sample)
sample <- sample[, -c(1,2,3,5,6,12)] ## For remove column Class
head(sample)

# lst <- as.data.frame(lapply(sample, function(x) round((x-min(x))/(max(x)-min(x)), 3))) 
lst <- as.data.frame(lapply(sample, function(x) round(x, 3)))
head(lst)
dataSample <- lst
head(dataSample)
kNNdistplot(dataSample, k = 5)

change_data <- 0.15
abline(h = change_data, col = "red", lty = 2) #----------------- Note
res <- dbscan(dataSample, eps = change_data, minPts = 5)
res
pairs(dataSample, col = res$cluster + 1L)
dataSample$cluster <- res$cluster
cleanall <- dataSample %>% filter(cluster > 0)
par(mfrow=c(1,2))
plot(cleanall$grvi, cleanall$CHM)
plot(dataSample$grvi, dataSample$CHM)

## Feature Selection
svrdata <- cleanall
svrdata <- cleanall[,-c(7)]
head(svrdata)
#library(Boruta)
# Decide if a variable is important or not using Boruta
boruta_output <- Boruta(CHM ~ ., data=na.omit(svrdata), doTrace=2)  # perform Boruta search
boruta_signif <- names(boruta_output$finalDecision[boruta_output$finalDecision %in% ("Confirmed")])  # collect Confirmed and Tentative variables
print(boruta_signif)  # significant variables
plot(boruta_output, cex.axis=.7, las=2, xlab="", main="Variable Importance")  # plot variable importance

# Divide data to training and testing ===============================
set.seed(3033)
intrain <- createDataPartition(y = svrdata$CHM, p= 0.7, list = FALSE)
training <- svrdata[intrain,]
testing <- svrdata[-intrain,]

dim(training)
dim(testing)
anyNA(svrdata)
## RMSE
rmse <- function(error)
{
  sqrt(mean(error^2))
}

# svr model ==============================================

model <- svm(CHM ~ . , training)
predictedY <- predict(model, testing)

error <- testing$CHM - predictedY  # 
svrPredictionRMSE <- rmse(error)  #  

# tuneResult <- tune(svm, frci ~ .,  data = training,
#                    ranges = list(epsilon = seq(0,1,0.1), cost = 2^(2:9)))

tuneResult <- tune(svm, CHM ~ .,  data = training,
                   ranges = list(epsilon = 0.1, cost = 1))
# print(tuneResult) 
# plot(tuneResult)

tunedModel <- tuneResult$best.model
tunedModelY <- predict(tunedModel, testing)
error <- testing$CHM - tunedModelY
tunedModelRMSE <- rmse(error)  # 2.219642

# 1. 'Actual' and 'Predicted' data
df <- data.frame(testing$CHM, tunedModelY)
# 2. R2 Score components
# 2.1. Average of actual data
avr_y_actual <- mean(df$testing.CHM)
# 2.2. Total sum of squares
ss_total <- sum((df$testing.CHM - avr_y_actual)^2)
# 2.3. Regression sum of squares
ss_regression <- sum((df$tunedModelY - avr_y_actual)^2)
# 2.4. Residual sum of squares
ss_residuals <- sum((df$testing.CHM - df$tunedModelY)^2)
# 3. R2 Score
r2 <- 1 - ss_residuals / ss_total

setwd("E:/00AllResult/00AllData/02102019_All Data CDN Join SMT/") #---------------------- After running
write.xlsx(cleanall, file = "AllData_Cidanau_SMT_57.19.xlsx")
# write.csv(cleanall, file = "FRCI_LINE7_78.13.csv")